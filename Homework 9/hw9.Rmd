...
\implies \ell(\sigma^2) &= \sum_{i=1}^n \log{P(Z_{t_i})} \\
  &= \sum_{i=1}^n \log{\frac{1}{\sqrt{2 \pi \sigma^2(t_{i} - t_{i-1})}} e^{-\frac{x_i^2}{2\sigma^2(t_{i} - t_{i-1})}}}\\
...
#
#   xi[6] <- 0
#   xi[5] <- rnorm(1, u, v*ct[6])
#   xi[4] <- xi[5] + rnorm(1, u, v*ct[5])
#   xi[3] <- xi[5] + rnorm(1, u, v*(ct[5]+ct[4]))
#   xi[2] <- xi[3] + rnorm(1, u, v*ct[3])
#   xi[1] <- xi[4] + rnorm(1, u, v*(ct[3]+ct[2]+ct[4]))
#
#   xL[1] <- xi[4] + rnorm(1, u, v*(ct[4]+ct[3]+ct[2]+ct[1]))
#   xL[4] <- xi[1] + rnorm(1, u, v*(ct[1]))
#   xL[7] <- xi[1] + rnorm(1, u, v*(ct[1]))
#   xL[2] <- xi[3] + rnorm(1, u, v*(ct[3]+ct[2]+ct[1]))
#   xL[3] <- xi[2] + rnorm(1, u, v*(ct[2]+ct[1]))
#   xL[6] <- xi[2] + rnorm(1, u, v*(ct[2]+ct[1]))
#   xL[5] <- rnorm(1, u, v*sum(ct))
#
#   return(list(xi=xi, xL=xL))
# }
```
...
```{r}
# computeSELogLikelihood(c(0, 1, 1), ccr5)
# computeSELogLikelihood(c(0, 1, .1), ccr5)
# computeSELogLikelihood(c(0, 1, 10), ccr5)
computeSELogLikelihood(ccr5, 1, 1, 1)
computeSELogLikelihood(ccr5, 10, 1, .1)
computeSELogLikelihood(ccr5, 1, .5, 100)
```
...
# r3 <- optim(par=c(10,10,10), fn=computeSELogLikelihoodOptim, f=ccr5)
# r4 <- optim(par=c(10,10,10), fn=computeSELogLikelihoodOptim, f=ccr5)
r1
...
The relatedness matrix (Child, Father, Mother) is:
