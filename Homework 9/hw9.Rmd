...
```{r}
walkRandomly <- function(steptimes, x0) {
  # x <- runif(steps)
  # x <- cumsum(c(x0, x))
  x <- rep(0, length(steptimes)+1)
  x[1] <- x0
  x[2] <- rnorm(1, 0, steptimes[1])
  for (i in 3:length(steptimes)) {
    x[i] <- rnorm(1, 0, steptimes[i]-steptimes[i-1]) + x[i-1]
  }
  return(x)
}
```
...
steptimes <- sort(runif(steps))
walk <- walkRandomlyUniform(steptimes, x0)
qqplot(steptimes, walk)
```
...
* Let $X(t)$ be conducting scaled Brownian motion with variance parameter
  ...
  ```{r}
walkRandomlyNormal <- function(nsteps, x0, m=0, v=1) {
  steps <- rnorm(steps, m, v)
  walk <- cumsum(c(x0, steps))
  # x <- rep(0, steps+1)
  # x[1] <- x0
  # for (i in 2:(steps+1)) {
  #   x[i] <- rnorm(1, m, v) + x[i-1]
  # }
  return(list(steps=steps, walk=walk))
}

likRandomlyNormal <- function(steps, m=0, v=1) {
  p <- pnorm(steps, m, v)
  logp <- log(p)
  return(sum(logp))
}
...
res <- walkRandomlyNormal(nsteps, x0)
steps <- res$steps
walk <- res$walk

qqplot(x=seq(0,nsteps), walk)

v <- seq(.1,10,.01)
logliks <- rep(0,length(v))

for (i in 1:length(v)) {
  res <- walkRandomlyNormal(nsteps, x0, v[i])
  logliks[i] <- likRandomlyNormal(res$steps, x0, v[i])
}
