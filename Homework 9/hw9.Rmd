...
sum(cov(runs)-covar)/sum(covar)
```

The results match. The covariance values are within an order of magnitude of
each other, and the relative deviation is around $.02$, so it is reasonable to
assume the simulation matches the expectation.
...
ones to nearly 0. Should an entry be distance from all other points, then the
